...
@Component({
  selector: 'app-trade-category-stats',
  templateUrl: './trade-category-stats.component.html',
  styleUrls: ['./trade-category-stats.component.css']
})

export class TradeCategoryStatsComponent implements OnInit {
  startdateValue: Date;
  enddateValue: Date;
  isShow = false;
  display1 = 'none';
  ...
  endDateConvert: any;
  onClickMe(start: any, end: any) {
    console.log(start.toDateString() + "-------------" + end);
    this.display1 = 'flex';
    this.startDateConvert = start.toLocaleDateString();
    this.endDateConvert = end.toLocaleDateString();
  }
